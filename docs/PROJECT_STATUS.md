...
```
amux serve (兄弟仓库 ../amux)      ← 轮询 tmux,提供 HTTP/WebSocket API
lib/ (Flutter)                      ← 跨平台客户端(iOS/Android/macOS/Linux/Windows/Web)
```

Rust 服务端(`amux` CLI,已从本仓库合并进兄弟仓库 `amux`;`full` cargo feature 承载 macOS 宿主扩展)跑在**每台被监控的 Mac**上;macOS 宿主 app 用 `--features full` 编译并内嵌它。Flutter 客户端跑在**任意设备**上,通过 HTTP/WS 连接服务端。
...
|---|---|---|
| Rust 服务端 | ✅ `apps/apple/AgentMonitorService/` | ➡️ 已合并进兄弟仓库 `amux`(`amux serve --features full`) |
